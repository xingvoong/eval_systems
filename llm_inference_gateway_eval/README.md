...
**The routing layer is hardened. The classifier is not.** Rule-based routing held 12/12 adversarial attacks. Exact comparisons have no attack surface. The zero-shot classifier failed 3/6 injection cases — one declarative preamble flipped the label to 0.99 confidence wrong.

**The learned router is the real system.** Phase 1 tested the wrong code path. `classify_prompt()` got 44%. `learned_router.py` hit 100%. Observe before you measure.

**Validate the judge before trusting it.** 0.93 Spearman correlation, zero variance, 0/5 adversarial fooled. Skip that step and you don't know what your judge is actually measuring.

**CI thresholds are floors, not goals.** The classifier threshold is 40% — not because that's good, but because that's the measured baseline. Thresholds catch regressions. They don't certify quality.

**Adversarial evals expose eval flaws first.** Phase 7 showed 6/12 "broke" on the first run. Wrong test expectations, not router failures. Fix the eval, rerun: 12/12.
